...
const DefaultContainer = <span></span>;
...
class Truncate extends React.PureComponent {
    ...
    getAvailableTextLength() {
        ...
        const singleLineWidthMinusPadding = parseFloat(style.width)
            - (parseFloat(style.paddingLeft) + parseFloat(style.paddingRight));

        const availableWidth = (lines * singleLineWidthMinusPadding) - extraWidth;

        const conversionTextWidth = this.canvasContext.measureText(conversionText).width;

        // Convert width into string length...
        // TODO Change the way we handle line endings to a more bullet proof method...
        return Math.floor((conversionTextLength * availableWidth) / conversionTextWidth) - ((lines * lineEndingTextLengthBuffer) + ellipsisWidth);
    }
        ...
        for (let i = 0; i < this.texts.length; i += 1) {
            if (this.isTruncated) {
                ...
                if (newAvailableTextLength < incrementedText.length) {
                    const remainingTextLength = newAvailableTextLength - previousTextLength;
    ...
    renderText() {
        ...
        for (let i = 0; i < childrenArray.length; i += 1) {
            ...
            content.push(textRenderer(...this.texts.slice(textStartingIndex, nextTextStartingIndex)));
            content.push(' ');
    ...
    render() {
        const {
            container: Container, containerId, ellipsis, ...rest
        } = this.props;
        ...
        return (
            ...
            >
                {this.renderText()}
                <span
                    ref={((instance) => { this.ellipsis = instance; })}
